...
/**
 * @author saleson
 * @date 2022-03-18 12:08
 */
public class FileMmapTest {
    public static void main(String[] args) throws Exception {

        URL url = Thread.currentThread().getContextClassLoader().getResource("file/cscw.txt");
        Path rfpath = Paths.get(url.toURI());
        Path fdirpath = rfpath.getParent();
        Path wfpath = Paths.get(fdirpath.toString(), "w.txt");

        //记录开始时间
        long start = System.currentTimeMillis();
        //通过RandomAccessFile的方式获取文件的Channel，这种方式针对随机读写的文件较为常用，我们用文件一般是随机读写
        RandomAccessFile randomAccessFile = new RandomAccessFile(wfpath.toFile(), "rw");
